...
cbmbasic implements a small plugin system that lets developers add additional statements, functions etc. Right now, you can turn this on with "SYS 1" (turn off with "SYS 0"), and use the new statements LOCATE _y_, _x_ (set cursor position), SYSTEM _string_ (run command line command) and the extended WAIT _port_, _mask_, which implements the [Bill Gates easter egg](http://www.pagetable.com/?p=43).
