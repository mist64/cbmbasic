...
static int get_console_info(HANDLE *h, COORD *pos, DWORD *size) {
  ...
  *h = GetStdHandle(STD_OUTPUT_HANDLE);
  if (*h == INVALID_HANDLE_VALUE ||
      GetConsoleScreenBufferInfo(*h, &info) == FALSE)
    return -1;
...
void set_color(int c) {
  ...
  bg = info.wAttributes & (BACKGROUND_RED | BACKGROUND_GREEN | BACKGROUND_BLUE |
  ...
  case COLOR_PURPLE:
    info.wAttributes =
        bg | FOREGROUND_RED | FOREGROUND_BLUE | FOREGROUND_INTENSITY;
    break;

  case COLOR_YELLOW:
    info.wAttributes =
        bg | FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY;
    break;
